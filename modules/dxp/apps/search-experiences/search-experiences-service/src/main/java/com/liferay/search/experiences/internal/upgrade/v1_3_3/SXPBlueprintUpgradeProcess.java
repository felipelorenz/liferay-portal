		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			for (FieldSet fieldSet : fieldSets) {
				...
				for (Field field : fields) {
					Map<String, String> helpTextMap = field.getHelpText_i18n();
					Map<String, String> labelMap = field.getLabel_i18n();
					...
					if (helpTextMap != null) {
						sxpElementJSONObject.put(
							...
							"helpText_i18n",
							JSONFactoryUtil.createJSONObject(helpTextMap)
						);
					}
					...
					if (labelMap != null) {
						sxpElementJSONObject.put(
							"label",
							labelMap.get(LocaleUtil.toLanguageId(LocaleUtil.US))
						).put(
							"label_i18n",
							JSONFactoryUtil.createJSONObject(labelMap)
						);
					}
				}
			}
